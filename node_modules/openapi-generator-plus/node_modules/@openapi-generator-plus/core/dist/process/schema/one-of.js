    ...
    if (result.discriminator) {
        for (const [addedApiSchema, addedSchema] of added) {
            if (!(0, types_1.isCodegenDiscriminatableSchema)(addedSchema)) {
                throw new Error(`oneOf "${result.name}" with discriminator references a non-discriminatable schema: ${(0, stringify_1.debugStringify)(addedApiSchema)}`);
            }
...
function toCodegenOneOfSchemaInterface(apiSchema, naming, state) {
    ...
    for (const oneOfApiSchema of oneOf) {
        ...
        if (!(0, types_1.isCodegenObjectSchema)(oneOfSchema) && !(0, types_1.isCodegenCompositionSchema)(oneOfSchema)) {
            /* Create a wrapper around this primitive type */
            const wrapper = (0, wrapper_1.createWrapperSchemaUsage)(oneOfSchema.serializedName || `${oneOfSchemaUsage.schema.schemaType.toLowerCase()}_value`, (0, openapi_type_guards_1.isOpenAPIReferenceObject)(oneOfApiSchema) ? null : result, oneOfSchemaUsage, oneOfApiSchema, state).schema;
            oneOfSchema = wrapper;
        }
        if (!(0, types_1.isCodegenObjectSchema)(oneOfSchema) && !(0, types_1.isCodegenCompositionSchema)(oneOfSchema) && !(0, types_1.isCodegenWrapperSchema)(oneOfSchema)) {
            throw new Error(`Failed to convert oneOf part to object schema: ${(0, stringify_1.debugStringify)(oneOfApiSchema)}`);
        }
        (0, utils_1.addImplementor)(result, oneOfSchema);
        added.push([oneOfApiSchema, oneOfSchema]);
    }
