...
function toCodegenEnumSchema(apiSchema, naming, state) {
    ...
    if (typeof apiSchema.type !== 'string') {
        throw new Error(`Invalid schema type for enum schema: ${apiSchema.type}: ${(0, stringify_1.debugStringify)(apiSchema)}`);
    }
    ...
    apiSchema.enum.forEach(name => {
        ...
        if (!literalValue) {
            state.log(types_1.CodegenLogLevel.WARN, `Cannot format literal for enum value "${name}" in ${(0, stringify_1.debugStringify)(apiSchema)}`);
            return;
        }
