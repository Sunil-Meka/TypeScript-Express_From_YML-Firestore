...
function toCodegenSchemaUsage(apiSchema, state, options) {
    ...
    const result = Object.assign(Object.assign({}, (0, utils_1.extractCodegenSchemaInfo)(schemaObject)), { required: options.required, schema: schemaObject, examples: null, defaultValue: null });
    ...
    result.defaultValue = (0, utils_1.toDefaultValue)(apiSchema.default, result, state);
    return result;
}
...
function toCodegenSchema(apiSchema, $ref, options, state) {
    ...
    const existing = (0, utils_2.findKnownSchema)(apiSchema, $ref, state);
    if (existing) {
        return existing;
    }
    ...
    const existingNow = (0, utils_2.findKnownSchema)(apiSchema, $ref, state);
    if (existingNow) {
        return existingNow;
    }
    let result;
    switch (schemaType) {
        ...
        case types_1.CodegenSchemaType.OBJECT:
            if (!naming) {
                ...
                throw new Error(`no name for ${(0, stringify_1.debugStringify)(apiSchema)}`);
            }
            result = (0, object_1.toCodegenObjectSchema)(apiSchema, naming, state);
            break;
        case types_1.CodegenSchemaType.INTERFACE:
            throw new Error(`Cannot create an interface directly from an OpenAPI schema: ${(0, stringify_1.debugStringify)(apiSchema)}`);
        case types_1.CodegenSchemaType.WRAPPER:
            throw new Error(`Cannot create a wrapper directly from an OpenAPI schema: ${(0, stringify_1.debugStringify)(apiSchema)}`);
        case types_1.CodegenSchemaType.HIERARCHY:
            throw new Error(`Cannot create a hierarchy directly from an OpenAPI schema: ${(0, stringify_1.debugStringify)(apiSchema)}`);
        ...
        case types_1.CodegenSchemaType.BINARY: {
            if (typeof apiSchema.type !== 'string') {
                throw new Error(`Unsupported schema type "${apiSchema.type}" for property in ${(0, stringify_1.debugStringify)(apiSchema)}`);
            }
...
function fixApiSchema(apiSchema, state) {
    ...
    */
    if (apiSchema.type === 'array' && apiSchema.enum) {
        if (apiSchema.items) {
            const items = (0, utils_1.resolveReference)(apiSchema.items, state);
            if (!items.enum) {
...
function discoverSchemasInOtherDocuments(testFunc, state) {
    ...
    for (const path of paths) {
        ...
        for (const schemaName in specSchemas) {
            const anApiSchema = (0, utils_1.resolveReference)(specSchemas[schemaName], state);
            if (testFunc(anApiSchema, state)) {
                /* We load the schema using a reference as we use references to distinguish between explicit and inline models */
                const reference = {
                    $ref: (0, utils_2.refForPathAndSchemaName)(path, doc, schemaName, state),
                };
