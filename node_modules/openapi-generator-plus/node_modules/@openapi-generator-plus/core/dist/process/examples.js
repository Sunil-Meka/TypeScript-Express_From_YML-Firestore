...
function exampleValue(value, mediaType, schema, state) {
    ...
    if (valueLiteral === null || valueString === null) {
        state.log(types_1.CodegenLogLevel.WARN, `Cannot format literal for example ${(0, stringify_1.debugStringify)(value)} in ${(0, stringify_1.debugStringify)(schema)}`);
        return null;
    }
    return {
        value,
...
function toCodegenExamples(apiExample, examples, mediaType, schema, state) {
    ...
    for (const mediaTypeOrName in examples) {
        const apiExample = (0, utils_2.resolveReference)(examples[mediaTypeOrName], state);
