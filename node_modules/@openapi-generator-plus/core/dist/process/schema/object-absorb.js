...
function absorbCodegenSchema(schema, target, options = {}) {
    ...
    if (schema.additionalProperties) {
        if (target.additionalProperties) {
            throw new Error(`Cannot absorb schema as the target already has additionalProperties: ${(0, stringify_1.debugStringify)(schema)}`);
        }
...
function absorbApiSchema(apiSchema, target, scope, state) {
    if (!(0, openapi_type_guards_1.isOpenAPIReferenceObject)(apiSchema)) {
        ...
        if (apiSchema.additionalProperties) {
            if (target.additionalProperties) {
                throw new Error(`Cannot absorb schema as the target already has additionalProperties: ${(0, stringify_1.debugStringify)(apiSchema)}`);
            }
    ...
    if ((0, types_1.isCodegenMapSchema)(schema)) {
        ...
        throw new Error(`Cannot absorb schema as it isn't an object: ${(0, stringify_1.debugStringify)(schema)}`);
    }
    return schema;
}
