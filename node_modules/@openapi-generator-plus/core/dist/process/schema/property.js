...
exports.createCodegenProperty = exports.addCodegenProperty = exports.toCodegenProperties = void 0;
...
function toCodegenProperties(apiSchema, scope, state) {
    ...
    const requiredPropertyNames = typeof apiSchema.required === 'object' ? [...apiSchema.required] : [];
    ...
    if (requiredPropertyNames.length > 0) {
        state.log(types_1.CodegenLogLevel.WARN, `Required properties [${requiredPropertyNames.join(', ')}] missing from properties: ${(0, stringify_1.debugStringify)(apiSchema)}`);
    }
...
function toCodegenProperty(name, apiSchema, required, scope, state) {
    ...
    return Object.assign(Object.assign({}, schemaUsage), { name: state.generator.toIdentifier(name), serializedName: name, description: description || schemaUsage.schema.description || null, initialValue: schemaUsage.defaultValue || state.generator.initialValue((0, utils_1.toCodegenInitialValueOptions)(schemaUsage)) || null, vendorExtensions: (0, vendor_extensions_1.toCodegenVendorExtensions)(apiSchema), discriminators: null });
}
function createCodegenProperty(name, schemaUsage, state) {
    const property = Object.assign(Object.assign({ name: state.generator.toIdentifier(name), serializedName: name, description: null }, (0, utils_1.extractCodegenSchemaUsage)(schemaUsage)), { initialValue: state.generator.initialValue((0, utils_1.toCodegenInitialValueOptions)(schemaUsage)) || null, vendorExtensions: null, discriminators: null });
