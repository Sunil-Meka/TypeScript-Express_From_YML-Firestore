...
function registerStandardHelpers(hbs, { generator, log, utils }) {
    ...
    hbs.registerHelper('inheritedProperties', function () {
        ...
        if (this.parents) {
            const parentProperties = core_1.idx.allValues((0, generator_common_1.uniquePropertiesIncludingInheritedForParents)(this.parents));
            if (this.properties) {
                const myProperties = this.properties;
                return parentProperties.filter(p => !core_1.idx.get(myProperties, p.name));
            }
            else {
    ...
    hbs.registerHelper('nonDefaultResponses', function () {
        ...
        if (this.responses) {
            return core_1.idx.allValues(this.responses).filter(response => !response.isDefault);
        }
        else {
            return [];
        }
